...
A *sticky* article is similar to a *live* article insofar as it can be viewed outside the Textpattern administration area but will _not_ appear in article lists unless the respective <code>article</code> or <code>article_custom</code> tag includes a <code>status</code> attribute with a <code>sticky</code> value.
