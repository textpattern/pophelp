h1. Textile: Inserted text

Use ==+pluses+== around text that has been +inserted+. This is useful in marking changes from one version of a document to the next.

The @<ins>@ HTML tag is applied:

bc. +a phrase+ → <ins>a phrase</ins>

p(alert-block information). *Note:* Quick tags must open and close within a single paragraph to produce valid markup.

h3. Textile documentation

"(link-external)More information on insertions and deletions in Textile":http://textpattern.com/textile-ins-and-del.

h3. HTML documentation (via MDN)

"(link-external)The @<ins>@ HTML element":https://developer.mozilla.org/en-US/docs/Web/HTML/Element/ins.
