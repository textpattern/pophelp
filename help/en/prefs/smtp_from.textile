...
* On UNIX/Mac OS servers, the entered email address is used as the 4th parameter (prefixed with "-f") for the PHP <code>mail()</code> function
* On Windows servers, the entered email address is used to set (or override) the <code>sendmail_from</code> php.ini setting
